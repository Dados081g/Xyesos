...
# Подменю "Категория" — пошаговое: каждый экран показывает только одну следующую
# кнопку (Oxide → Android-noRoot → cry4me → цены). Стиль/цвет кнопки есть только
# у ReplyKeyboardButton, поэтому это нижние панели, а не инлайн-кнопки.
...
ANDROID_NOROOT_KB = ReplyKeyboardMarkup(
    keyboard=[
...
async def category_android_noroot(message: Message):
    text = "🛍 Выберите категорию:"
    entities = [emoji_entity(text, "🛍", EMOJI_CATEGORY_TEXT)]
    await message.answer(text, entities=entities, reply_markup=ANDROID_NOROOT_KB)
